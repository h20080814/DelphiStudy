...
begin
  if Num2 = 0 then begin
    raise EMathException.Create('除数不能为0,自定义');    //子类抛出一个异常
    raise Exception.Create('除数不能为0');      //父类抛出一个异常,等同于Result:=Exception.Create ('除数不能为0');Exit;
  end;
end;
...
begin
  ...
  try
    ...
    { TODO -oUser -cConsole Main : Insert code here }
      ...
      on E{声明一个变量}: EMathException{类型} do   //异常子类
        Writeln(E.ClassName, ': ', E.Message);
      on E{声明一个变量}: Exception{类型} do   //异常父类
        Writeln(E.ClassName, ': ', E.Message);
    end;
  finally
    //释放资源
