...
type
  ...
  private
    FConsumption: TProduct;
  protected
    procedure Execute; override;
  public
    constructor create; overload;
    constructor create(Consumption: TProduct); overload;
  end;
...
constructor TConsumptionTread.create(Consumption: TProduct);  //消费有参构造
begin
  inherited create(False);
  Self.FConsumption := Consumption;
end;
...
begin
  while True do begin
    ...
    System.TMonitor.Enter(Self.FConsumption);  //进入临界区，放入循环体内可实现多条生产线集中管理
    if Self.FConsumption.IsConsumption then   // 当是true时表示有产品
    begin
      Str := '消费线程ID ' + Self.ThreadID.ToString + ', 当前产品编号：' + self.FConsumption.Id.ToString;
      Form1.mmo1.Lines.Add(Str);
      Self.Sleep(1000);   //等待100毫秒
      Self.FConsumption.IsConsumption := False; // 产品已消费待生产

    end;
    System.TMonitor.Pulse(Self.FConsumption); //通知生产线程开始生产，pulse唤醒随机一个生产线程
    System.TMonitor.Wait(Self.FConsumption, INFINITE); //让线程处于等待状态，INFINITE（无限）
    System.TMonitor.Exit(Self.FConsumption);   //退出临界区
